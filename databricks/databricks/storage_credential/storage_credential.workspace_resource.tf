resource "databricks_storage_credential" "workspace_storage_credential" {
  for_each = { for key, value in var.databricks_storage_credential_data : key => value if value.enabled && value.provider_scope == "workspace" }
