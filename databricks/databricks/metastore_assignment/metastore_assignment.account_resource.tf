resource "databricks_metastore_assignment" "account_metastore_assignment" {
  ...
  metastore_id = coalesce(
    try(data.databricks_metastore.account_metastore_region["${each.key}"].id, null), # Already existing and using region to look up
    try(data.databricks_metastore.account_metastore_name["${each.key}"].id, null),   # Already existing and using name to look up
