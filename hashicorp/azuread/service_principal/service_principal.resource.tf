...
resource "azuread_service_principal" "service_principal" {
  for_each = { for key, value in var.azuread_service_principal_data : key => value if value.enabled }

  # Required Arguments
  client_id = coalesce(
    try(each.value.client_id, null),
    try(var.azuread_application_registration_output["${each.value.app_registration_name}"].client_id, null),
    try(var.azuread_application_output["${each.value.app_registration_name}"].client_id, null),
  )
